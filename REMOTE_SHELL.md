...
3. 点击 **上传**。支持 File System Access API 的浏览器会直接打开所有文件选择器；其他手机浏览器会先显示上传窗口，可分别点击 **打开文件管理器**（文档、压缩包、代码、模型等）或 **照片 / 视频 / 音频**。桌面端也可直接把任意文件拖到 Files 面板。
...
- **手机只显示照片或视频**：强制刷新网页以加载最新静态资源；点击 **上传** 后使用 **打开文件管理器**，不要使用旁边的媒体入口。旧版手机浏览器如仍弹出来源菜单，请在系统面板中点 **更多** 进入文件管理器。
