...
const elements = {
  ...
  herdrSessionPicker: byId("herdr-session-picker"),
  herdrSessionButton: byId("herdr-session-button"),
  herdrSessionLabel: byId("herdr-session-label"),
  herdrSessionMenu: byId("herdr-session-menu"),
...
function agentLabel(agent) {
  return agent?.label || agent?.workspace_label || agent?.project || agent?.agent || agent?.pane_id || "Agent";
}
...
function herdrSessionSources() {
  return Array.isArray(state.herdrSessions?.sources)
    ? state.herdrSessions.sources
    : [];
}

function handleHerdrSessions(message) {
  state.herdrSessions = {
    ...message,
  ...
  renderHerdrSessionPicker();
  if (!state.agents.length) renderDashboard();
}

function setHerdrSessionMenuOpen(open) {
  const shouldOpen = Boolean(open) && !elements.herdrSessionPicker.hidden;
  elements.herdrSessionMenu.hidden = !shouldOpen;
  elements.herdrSessionButton.setAttribute("aria-expanded", String(shouldOpen));
}

function renderHerdrSessionPicker() {
  const sources = herdrSessionSources();
  const totalSessions = sources.reduce(
    (total, source) => total + source.sessions.length,
    0,
  );
  const visible = totalSessions >= 2;
  elements.herdrSessionPicker.hidden = !visible;
  if (!visible) {
    setHerdrSessionMenuOpen(false);
    elements.herdrSessionMenu.replaceChildren();
    return;
  }

  const multiSource = sources.length > 1;
  const singleSource = sources[0];
  const activeName = singleSource?.active || "default";
  elements.herdrSessionLabel.textContent = state.sessionSwitchPending
    ? "切换中…"
    : (multiSource ? "Sessions" : activeName);
  elements.herdrSessionButton.disabled = state.sessionSwitchPending
    || state.promptPending
    || Boolean(state.agentKeyPending)
    || state.interruptPending
    || state.startPending
  ...
  const groups = sources.map((source) => {
    ...
    group.className = "herdr-session-group";
    if (multiSource) {
      const heading = document.createElement("span");
      heading.className = "herdr-session-source";
      heading.textContent = source.label || source.host || source.source_id || "Agent Source";
      group.append(heading);
    }
    const selectedName = source.active || "default";
    for (const session of source.sessions) {
      const option = document.createElement("button");
      option.type = "button";
      option.className = `herdr-session-option${session.running ? "" : " is-stopped"}`;
      option.setAttribute("role", "option");
      option.setAttribute("aria-selected", String(session.name === selectedName));
      option.disabled = state.sessionSwitchPending;

      const check = document.createElement("span");
      check.className = "herdr-session-check";
      check.setAttribute("aria-hidden", "true");
      check.textContent = session.name === selectedName ? "✓" : "";
      const name = document.createElement("span");
      name.className = "herdr-session-name";
      name.textContent = session.name;
      const status = document.createElement("span");
      status.className = "herdr-session-state";
      status.textContent = session.running ? "运行中" : "已停止";
      option.append(check, name, status);
      option.addEventListener("click", () => switchHerdrSession(source, session.name));
      group.append(option);
    }
    return group;
  });
  elements.herdrSessionMenu.replaceChildren(...groups);
}

function switchHerdrSession(source, sessionName) {
  if (state.sessionSwitchPending) return;
  if (state.promptPending || state.agentKeyPending || state.interruptPending || state.startPending) {
    showToast("暂时无法切换", "请等待当前 Agent 操作完成后再切换 Session。", "error");
    return;
  }
  const selectedName = source.active || "default";
  if (sessionName === selectedName) {
    setHerdrSessionMenuOpen(false);
    return;
  }
  const requestId = `session-${Date.now()}-${Math.random().toString(16).slice(2)}`;
  if (!send({
    type: "session_switch",
    source_id: source.source_id || source.host,
    host: source.host,
    session: sessionName,
  ...
  state.sessionSwitchRequestId = requestId;
  state.agents = [];
  state.activePane = null;
  state.outputs.clear();
  state.ansiOutputs.clear();
  state.outputUpdatedAt.clear();
  state.outputRenderedPane = null;
  state.outputRenderedSnapshot = null;
...
function replaceAgentSnapshot(agents) {
  ...
  const completedSessionSwitch = state.sessionSwitchPending;
  state.sessionSwitchPending = false;
  state.sessionSwitchRequestId = "";
  ...
  if (changed || completedSessionSwitch) renderAll();
  if (completedSessionSwitch) renderHerdrSessionPicker();
...
function handleRelayError(payload) {
  ...
  if (
    ...
    && (
      payload?.request_id === state.sessionSwitchRequestId
      || !payload?.request_id
    )
  ) {
    state.sessionSwitchPending = false;
    ...
    renderHerdrSessionPicker();
    renderDashboard();
    showToast("Session 切换失败", message, "error");
    return;
  }
  if (
...
function createAgentListEmpty() {
  ...
  if (state.sessionSwitchPending) {
    title.textContent = "正在切换 Session";
    detail.textContent = "Relay 正在清理旧 Pane 状态并重新读取 Agent。";
  } else if (!state.agents.length) {
    const sources = herdrSessionSources();
    const onlySource = sources.length === 1 ? sources[0] : null;
    const selectedName = onlySource?.active || "default";
    const selectedSession = onlySource?.sessions.find(
      (session) => session.name === selectedName,
    );
    if (selectedSession && !selectedSession.running) {
      title.textContent = `Session “${selectedName}” 未运行`;
      detail.textContent = "可以从页面右上角切换到正在运行的 Herdr Session。";
    } else {
...
function bindEvents() {
  ...
  elements.herdrSessionButton.addEventListener("click", (event) => {
    event.stopPropagation();
    setHerdrSessionMenuOpen(elements.herdrSessionMenu.hidden);
  });
  elements.herdrSessionMenu.addEventListener("click", (event) => event.stopPropagation());
  document.addEventListener("click", () => setHerdrSessionMenuOpen(false));
  document.addEventListener("keydown", (event) => {
    if (event.key === "Escape") setHerdrSessionMenuOpen(false);
  });
