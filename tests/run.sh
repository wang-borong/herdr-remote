...
grep -q 'id="herdr-session-button"' "$WEB" && \
grep -q "session_switch" "$WEB_JS" && \
grep -q "handleHerdrSessions" "$WEB_JS"
assert_eq "$?" "0" "web app has session selector"

echo "9c. web app has no duplicate function declarations"
DUP_FUNCS=$(grep -oE '^[[:space:]]*function [A-Za-z0-9_]+\(' "$WEB_JS" | grep -oE '[A-Za-z0-9_]+\(' | sort | uniq -d)
[ -z "$DUP_FUNCS" ]
