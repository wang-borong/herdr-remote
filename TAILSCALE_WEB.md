...
- 按状态筛选和搜索本机 Agent。
- 查看最近 200、500 或 1000 行 Pane 输出，并可自动刷新；右侧提供不挤占正文的窄型触控滚动轨，以及一键回到最上方、最下方的按钮。xterm 自带滚动条会被关闭，避免出现双滚动条。支持触屏且提供 xterm 可访问性文本的浏览器会启用系统原生文本选择，并使用与 xterm 缓冲区同步、不会被虚拟行复用的稳定选择层。Android/Chrome 长按拖动并松手后会保留准确选区与系统操作栏，可继续使用复制、全选、分享、网页搜索等操作（实际菜单由系统决定）；选错后轻点输出即可取消，再次长按便可重新选择。不支持原生选择时，会回退到长按拖动后打开网页复制窗口。长按尚未形成选区及选区存在期间都会暂缓重绘，取消选择后自动补上最新输出；无选区时复制入口仍可打开完整的纯文本视图。Herdr 当前的 recent 快照接口最多返回约 1000 行，因此网页将 1000 标记为当前最大值；更完整的终端历史需要 Herdr 上游先提供无截断读取接口。
...
- Remote Shell 明确授权的用户可在 Files 当前目录上传文件：支持新文件系统 API 的浏览器直接打开所有文件选择器，其他手机浏览器提供独立的 **打开文件管理器** 与媒体入口，桌面端也可拖放任意文件；默认自动编号避免覆盖，也可显式选择原子覆盖。单文件默认上限为 2 GiB，可通过 `HERDR_WORKSPACE_UPLOAD_MAX_BYTES` 调整。
...
手机页面底部可在 Agents、终端和文件之间切换。Files 页面先显示主机与文件列表，点击文件后进入全屏阅读器，左上角返回文件列表；代码、Markdown 和 HTML 可复制原文，HTML 还可在安全渲染与源码模式间切换，其他普通文件可直接下载。拥有 Remote Shell 权限时，进入具体目录后可上传文件；手机兼容界面将文件管理器与照片/视频/音频入口分开，避免系统只显示媒体来源，PC 端同时支持把任意文件直接拖到 Files 面板。Agent 页面中，普通发送会立即提交 Prompt；Codex Prompt 可通过图片按钮、剪贴板粘贴或拖放附加 PNG、JPEG、WebP 图片。当 Agent 正在工作时，纯文本的 **Tab 缓存**按钮会将内容放入 Codex 队列，等待当前任务完成后处理。左上角返回按钮回到 Dashboard。新建 Agent 按钮在顶栏始终可用。
