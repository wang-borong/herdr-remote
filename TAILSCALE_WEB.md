...
- 按状态筛选和搜索本机 Agent。
- 查看最近 60、120 或 200 行 Pane 输出，并可自动刷新；支持触屏且提供 xterm 可访问性文本的浏览器会启用系统原生文本选择，并使用与 xterm 缓冲区同步、不会被虚拟行复用的稳定选择层。Android/Chrome 长按拖动并松手后会保留准确选区与系统操作栏，可继续使用复制、全选、分享、网页搜索等操作（实际菜单由系统决定）；选错后轻点输出即可取消，再次长按便可重新选择。不支持原生选择时，会回退到长按拖动后打开网页复制窗口。长按尚未形成选区及选区存在期间都会暂缓重绘，取消选择后自动补上最新输出；无选区时复制入口仍可打开完整的纯文本视图。
